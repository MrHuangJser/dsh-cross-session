...
const PLUGIN_NAME = "@mrhuangjser/dsh-cross-session";
...
var MissingServiceError = class extends Error {
	...
	constructor(service, purpose) {
		super(`@mrhuangjser/dsh-cross-session requires the "${service}" Host service (${purpose}), but this deployment does not compose it. Add the plugin that provides "${service}" to the host composition, or disable this plugin.`);
...
function defineSendTool(context) {
	return {
		...
		execute: async (rawArgs, exec) => {
			...
			let senderTitle;
			try {
				senderTitle = (await context.query.readTitle(callerId))?.title;
			} catch {
				senderTitle = void 0;
			}
			const request = {
				sender: senderTitle === void 0 ? { sessionId: callerId } : {
					sessionId: callerId,
					title: senderTitle
				},
				targetSessionId,
...
function buildSectionSchema(Schema) {
	return Schema.object({
		enabled: Schema.boolean().default(DEFAULT_SETTINGS.enabled).description("Register the cross-session tools at all."),
		defaultSendMode: Schema.union(["queue", "steer"]).default(DEFAULT_SETTINGS.defaultSendMode).description("Delivery mode used when sessions_send omits mode: queue or steer."),
		allowResume: Schema.boolean().default(DEFAULT_SETTINGS.allowResume).description("Allow a send to wake a cold session. Off means sessions_read stays read-only and nothing is ever resumed."),
		frameMessages: Schema.boolean().default(DEFAULT_SETTINGS.frameMessages).description("Prefix delivered messages with a frame naming the sending session, so the receiver can tell a peer from the human."),
		includeInjectedByDefault: Schema.boolean().default(DEFAULT_SETTINGS.includeInjectedByDefault).description("Include harness-injected context (system reminders, memory snapshots) when reading another session."),
		maxListResults: Schema.number().default(DEFAULT_SETTINGS.maxListResults).description("Upper bound on rows sessions_list returns."),
		maxReadMessages: Schema.number().default(DEFAULT_SETTINGS.maxReadMessages).description("Upper bound on messages sessions_read returns."),
		maxMessageChars: Schema.number().default(DEFAULT_SETTINGS.maxMessageChars).description("Per-message character budget before sessions_read truncates."),
		maxSendChars: Schema.number().default(DEFAULT_SETTINGS.maxSendChars).description("Upper bound on the body length sessions_send delivers.")
	});
}
function apply(ctx, config = {}) {
	...
	try {
		...
		Schema = "default" in loaded && typeof loaded.default === "function" ? loaded.default : loaded;
	} catch (error) {
